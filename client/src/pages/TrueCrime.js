...
const TrueCrime = () => {
    const [trueCrimePodcastData, setPodcastData] = useState('');
    useEffect(() => {
        fetch('/api/trueCrime')
            .then(res => res.json())
            .then(data => {
    ...
    let loading = true;
    if (trueCrimePodcastData[0] !== undefined) {
        loading = false;
    }
    ...
    return (
        <ThemeProvider theme={theme}>
            ...
            >
                <p className="wYl-font">TRUE CRIME</p>
            ...
            >
                ...
                >
                    {trueCrimePodcastData.map((podcast) => {
                        return (
